...
export default function socketServer(){
	console.log('Iniciando servidor de sockets')
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('c:pj', (data, {idSala, para}) => {
      // entiendase que si no hay 'para', el pj es para el dj de la sala
      // si hay 'para', el pj es para ese jugador
      const cliente = para ? socketsMap.get(para) : socketsDj.get(idSala)
			if (cliente){
				cliente.emit('s:pj', data, {de: socket.handshake.auth.token });
			} else{
				socket.emit('s:error', 'Jugador no conectado')
			}
    });
  });
}
