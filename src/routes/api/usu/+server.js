export const PATCH = async event => {
  ...
  try{
    switch(event.request.headers.get('accion')){
      case 'mailVis':   // cambiar la visibilidad del email del usuario
        record = await event.locals.pb.collection('users').update(event.locals.user.id, JSON.stringify(data));
        return new Response(JSON.stringify({type: 'success', message: 'Visibilidad cambiada.', value: record.emailVisibility}), {status: 200})        
